  ...
  >
    <div v-if="roomStore.page === 'confirm'" class="absolute top-5 right-5">
      <div class="flex items-center">
        ...
        <button
          class="ml-4 text-[16px] transition"
        ...
        <button
          class="ml-4 mr-5 text-[16px] transition"
        ...
        <current-user v-if="userStore.auth" />
      </div>
    </div>
...
import OutMessage from '@components/room/OutMessage.vue'
...
const getMembers = async () => {
  try {
    ...
    roomStore.members = await axios.get(
      `/smileeye/list-member/goal_root_id=${goalId}`
    )
  } catch (e) {
    //
  }
  ...
  if (/^\d*-\d*-\d*/.test(route.params.id as string)) {
    ...
    try {
      const result: any = await axios.get(
        `/smileeye/detail-goal/goal_id=${goalId}`
      )
      if (result) {
...
const debouncedUpdateMedia = useDebounceFn(async () => {
  await dbSet(
    dbRef(getDatabase(), `room/${route.params.id}/media/${userStore.user?.id}`),
    {
      user: {
